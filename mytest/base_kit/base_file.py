...
def json2file(data, save_path, save_name):
    ...
    :param save_name: 保存的文件名 xxx.json
    """
    # 存盘路径检查
...
def str_list2csv(data, save_path, save_name):
    ...
    格式:['str1_1,str1_2,str1_3','str2_1,str2_2,str2_3',
    ...
    dir_chk_create(save_path)
    # 文件写入
    with open(save_path + save_name, 'a', newline='', encoding='utf_8') \
            as txt_file:
        for item in data:
...
def dict_list2csv(data):
    ...
    """
    dir_chk_create(data.file_path)

    # 如果文件不存在，则创建表头的开关
    file_exist = os.path.exists(data.file_path + data.file_name)

    with open(data.file_path + data.file_name,
              'a', newline='', encoding='utf_8') as f:
